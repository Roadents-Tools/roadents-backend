...
object StationRouteBuilder {

  private final val LimitScale = 1.0

  def buildStationRouteList(req: StationRouteBuildRequestScala): Seq[Route] = {
    val rval = mutable.LinkedHashMap[Int, Route]()
    rval.put(locationTag(req.start), new Route(req.start, req.starttime))
  ...
  @inline
  private def genStartRoutes(req: StationRouteBuildRequestScala): Seq[Route] = {
  ...
  private def getStartNodes(req: StationRouteBuildRequestScala): Seq[StartWalkStep] = {
    TransitDataRetriever.getStartingStations(req.start, req.delta.total_max.avgWalkDist, req.walkLimit)
      .map(stat => {
        ...
        val walkTime = TimeDelta((walkMinutes * 60000).asInstanceOf[Long])
        StartWalkStep(req.start, stat, walkTime)
      })
  }

  @inline
  private def nextLayer(curlayer: Seq[Route], req: StationRouteBuildRequestScala) = {
  ...
  @inline
  private def genTransferRoutes(curlayer: Seq[Route],
                                req: StationRouteBuildRequestScala) = genTransferNodes(curlayer, req)
    .flatMap({ case (route, possiblesteps) => possiblesteps.map(step => route + step) })

  @inline
  private def genTransferNodes(curlayer: Seq[Route], req: StationRouteBuildRequestScala) = {
    ...
    val rval = curlayer
      ...
      .map(rt => rt -> res(reqsMap(rt)).toStream
        ...
        .map(st => TransferWalkStep(rt.currentEnd.asInstanceOf[Station], st, rt.currentEnd.distanceTo(st).avgWalkTime))
      )
    rval
  }

  @inline
  private def buildTransferReq(rt: Route, req: StationRouteBuildRequestScala): TransferRequest = {
    val usableDist = Seq(
      req.transferTime.total_max - rt.transferTime,
      req.transferTime.max,
      req.delta.total_max - rt.totalTime
    ).min.avgWalkDist
    TransferRequest(rt.currentEnd.asInstanceOf[Station], usableDist, req.walkLimit)
  }
  ...
  @inline
  private def genTransitRoutes(curlayer: Seq[Route], req: StationRouteBuildRequestScala): Seq[Route] = {
    ...
    val pathReqs = curlayer
      ...
      .map(rt => {
        ...
        rt -> PathsRequest(rt.currentEnd.asInstanceOf[Station], rt.endTime, usableTime, pathsLimit)
      })
      .toMap
  ...
  @inline
  private def buildArrivalReq(route: Route, chaininfo: StationWithRoute, req: StationRouteBuildRequestScala,
                              curlayer: Int): ArrivableRequest = {
