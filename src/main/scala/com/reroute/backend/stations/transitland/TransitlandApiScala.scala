...
import com.reroute.backend.model.location.LocationPointScala
...
class TransitlandApiScala {
  ...
  def getFeedsInArea(center: LocationPointScala, range: DistanceScala, restrict: util.Map[String, String],
                     avoid: util.Map[String, String]): util.List[URL] = {
    var url = TransitlandApiScala.BASE_FEED_URL + "?per_page=500"
    if (center != null && range != null && range.in(DistanceUnitsScala.METERS) >= 0) {
      val latVal1 = center.latitude - range.in(DistanceUnitsScala.MILES) * TransitlandApiScala.MILES_TO_LAT
      val latVal2 = center.latitude + range.in(DistanceUnitsScala.MILES) * TransitlandApiScala.MILES_TO_LAT
      val lngVal1 = center.longitude - range.in(DistanceUnitsScala.MILES) * TransitlandApiScala.MILES_TO_LONG
      val lngVal2 = center.longitude + range.in(DistanceUnitsScala.MILES) * TransitlandApiScala.MILES_TO_LONG
